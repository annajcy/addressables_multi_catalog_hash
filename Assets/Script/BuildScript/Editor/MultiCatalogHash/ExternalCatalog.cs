...
namespace Script.BuildScript.Editor.MultiCatalogHash
{
	...
	public class ExternalCatalog : ScriptableObject
	{
		...
		/// </summary>
		/// <param name="catalogDataEntry">The data entry location.</param>
		...
		public bool IsPartOfCatalog(ContentCatalogDataEntry catalogDataEntry, AddressableAssetsBuildContext aaContext)
		{
			// Don't bother if the asset groups is empty.
			if (assetGroups == null || assetGroups.Count <= 0)
				return false;

			if (catalogDataEntry.ResourceType == typeof(IAssetBundleResource))
			{
				AddressableAssetEntry entry = aaContext.assetEntries.Find(ae => string.Equals(ae.BundleFileId, catalogDataEntry.InternalId));
				if (entry != null)
					return assetGroups.Exists(ag => ag.entries.Contains(entry));

				// If no entry was found, it may refer to a folder asset.
				return assetGroups.Exists(ag => ag.entries.Any(e => e.IsFolder && string.Equals(e.BundleFileId, catalogDataEntry.InternalId)));
			}

			return assetGroups.Exists(ag => ag.entries.Any(e => (e.IsFolder && e.SubAssets.Any(a => catalogDataEntry.Keys.Contains(a.guid))) || catalogDataEntry.Keys.Contains(e.guid)));
		}
